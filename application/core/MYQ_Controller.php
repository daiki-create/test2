...
class MYQ_Controller extends MY_Controller {
     ...
     * バリデーション
     */
    protected function validate($validation_group='common', $requires=array())
    {
        log_debug("MYWWW_Controller.validate({$validation_group}) run.");
        if (parent::validate($validation_group, $requires))
        {
    ...
    public function _output($output)
    {
        ...
        elseif (empty($output))
        {
            ...
            if (file_exists(APPPATH . "../htdocs/js/{$this->_module}/{$this->_class}.js"))
                $this->_option_foot .= '<script type="text/javascript" src="/js/'.$this->_module.'/'.$this->_class.'.js?' . $this->_revision . '"></script>'."\n";

            if (file_exists(APPPATH . "../htdocs/js/{$this->_module}/{$this->_class}/{$this->_action}.js"))
                $this->_option_foot .= '<script type="text/javascript" src="/js/'.$this->_module.'/'.$this->_class.'/'.$this->_action.'.js?' . $this->_revision . '"></script>'."\n";
    ...
    protected function js_data($js_data=NULL)
    {
        ...
        elseif (is_array($js_data))
        {
            ...
            {
                $this->_js_data .= "VITAMINC.{$key} = " . json_encode($data) . ";\n";
            }
        }
